...
class PlayerReadCourtSchedulesTest {
    ...
    void test1() {
        courtScheduler.createScheduleSlot(
                new Court(1L, "Arthur Ashe"), TIME_SLOT_FOR_NOW, List.of(DayOfWeek.MONDAY));
        final var reservationSystem = new ReservationSystem(courtScheduler);

        assertThat(reservationSystem.getFreeScheduleSlots()).hasSize(1);
        assertThat(reservationSystem.getFreeScheduleSlots().get(0))
                .isEqualTo(new CourtScheduleSlot(new Court(1L, "Arthur Ashe"), TIME_SLOT_FOR_NOW));
    }

    @Test
    @DisplayName("Returns correct free daily schedule slots from the court scheduler for a 1 week requested time interval")
    void test2() {
        courtScheduler.createScheduleSlot(
                new Court(1L, "Arthur Ashe"), TIME_SLOT_FOR_NOW, List.of(DayOfWeek.MONDAY));

        final var today = LocalDate.now();
        final var dailyScheduleSlots =
                courtScheduler.getDailyScheduleSlots(
                        today.minus(7, ChronoUnit.DAYS), today);
